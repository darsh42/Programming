...
void flag_write(struct cpu *cpu, bool Z, bool N, bool H, bool C, uint8_t mask) {
    cpu->AF.lower = (cpu->AF.lower & (mask << 4)) | ((Z << 7) & (N << 6) & (H << 5) & (C << 4));
}

void cpu_init(struct cpu *GBcpu, uint8_t checksum) {
    cpu = GBcpu;
